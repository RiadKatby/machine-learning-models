...
class Env:
    ...
    evaulate_script_path: Optional[str] = os.environ.get("EVALUATE_SCRIPT_PATH")
    ...
    allow_run_cancel: Optional[str] = os.environ.get("ALLOW_RUN_CANCEL", "true")
    aml_env_name: Optional[str] = os.environ.get("AML_ENV_NAME")
    aml_env_train_conda_dep_file: Optional[str] = os.environ.get("AML_ENV_CONDA_DEP_FILE", "conda_dependencies.yml")
    rebuild_env: Optional[bool] = os.environ.get("AML_REBUILD_ENVIRONMENT", "false").lower().strip() == "true"

    use_gpu_for_scoring: Optional[bool] = os.environ.get("USE_GPU_FOR_SCORING", "false").lower().strip() == "true"
    aml_env_score_conda_dep_file: Optional[str] = os.environ.get("AML_ENV_SCORE_CONDA_DEP_FILE", "conda_dependencies_scoring.yml")
    aml_env_scorecopy_conda_dep_file: Optional[str] = os.environ.get("AML_ENV_SCORECOPY_CONDA_DEP_FILE", "conda_dependencies_scorecopy.yml")
    vm_size_scoring: Optional[str] = os.environ.get("AML_COMPUTE_CLUSTER_CPU_SKU_SCORING")
    compute_name_scoringL Optional[str] = os.environ.get("AML_COMPUTE_CLUSTER_NAME_SCORING")
    vm_priority_scoring: Optional[str] = os.environ.get("AML_CLUSTER_PRIORITY_SCORING", "lowpriority")
    min_nodes_scoring: int = int(os.environ.get("AML_CLUSTER_MIN_NODES_SCORING", 0))
    max_nodes_scoring: int = int(os.environ.get("AML_CLUSTER_MAX_NODES_SCORING", 4))
    rebuild_env_scoring: Optional[bool] = os.environ.get("AML_REBUILD_ENVIRONMENT_SCORING", "false").lower().strip() == "true"
    ...
    aml_env_name_scoring Optional[str] = os.environ.get("AML_ENV_NAME_SCORING")
    aml_env_name_score_copy Optional[str] = os.environ.get("AML_ENV_NAME_SCORE_COPY")
    batchscore_script_path Optional[str] = os.environ.get("BATCHSCORE_SCRIPT_PATH")
    batchscore_copy_script_path Optional[str] = os.environ.get("BATCHSCORE_COPY_SCRIPT_PATH")
